...
import {
  Package, ScanBarcode, Bell, BarChart3, Boxes, ShieldCheck,
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden bg-gradient-hero">
      ...
      >
        ...
        <path fill="url(#waveGrad1)" opacity="0.85" filter="url(#waveSoftBlur)"
        ...
        <path fill="url(#waveGrad2)" filter="url(#waveSoftBlur)"
      ...
      <div className="relative w-full max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        <div className="animate-fade-up text-center lg:text-left">
          ...
          <p className="text-lg text-muted-foreground mb-8 max-w-lg mx-auto lg:mx-0">
            Parmako helps pharmacies track inventory, organize bins, and prevent expirations — straight from your phone. Faster counts, fewer losses, healthier shelves.
          </p>
          <div className="flex flex-wrap gap-3 justify-center lg:justify-start">
            <a href="#" className="group flex items-center gap-3 px-5 py-3 rounded-xl bg-foreground text-background hover:opacity-90 transition-smooth shadow-soft">
          ...
          <div className="flex items-center gap-6 mt-10 text-sm text-muted-foreground lg:text-white/90 justify-center lg:justify-start">
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_,i)=><Star key={i} className="w-4 h-4 fill-primary text-primary lg:fill-white lg:text-white"/>)}
        ...
        <div className="relative h-[640px]">
          <div className="phone-stack-item"><StockScreen /></div>
          <div className="phone-stack-item"><BinsScreen /></div>
          <div className="phone-stack-item"><AlertsScreen /></div>
        </div>
      </div>
    </section>
  );
...
const features = [
  { icon: ScanBarcode, title: "Barcode scanning", desc: "Scan medicines instantly with your phone camera. No extra hardware required." },
  { icon: Boxes, title: "Smart bin management", desc: "Assign locations, organize shelves, and find anything in seconds." },
  { icon: Bell, title: "Expiry alerts", desc: "Get notified before stock expires. Never lose money to dead inventory again." },
  { icon: BarChart3, title: "Real-time analytics", desc: "Visualize stock movement, top sellers, and reorder needs at a glance." },
  { icon: ShieldCheck, title: "Audit-ready logs", desc: "Every change tracked. Stay compliant with full traceability." },
  { icon: Package, title: "Reorder automation", desc: "Auto-flag low stock and generate purchase lists in one tap." },
];
    ...
    <section id="features" className="min-h-screen flex items-center py-20 px-6">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-muted-foreground text-lg">Built with pharmacists, for pharmacists. Parmako replaces spreadsheets and paper logs with one beautifully simple app.</p>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {features.map(f => (
            <div key={f.title} className="group p-5 md:p-8 rounded-2xl bg-card border border-border hover:shadow-elegant hover:-translate-y-1 transition-smooth">
...
const steps = [
  { icon: Smartphone, title: "Download Parmako", desc: "Install on iOS or Android in under a minute." },
  { icon: ScanBarcode, title: "Scan your stock", desc: "Bulk-scan medicines and assign them to bins." },
    ...
    <section id="how" className="min-h-screen flex items-center py-20 px-6 bg-secondary/40">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg">No training. No setup fees. Just open the app.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {steps.map((s,i) => (
            <div key={s.title} className="relative p-8 rounded-2xl bg-card border border-border shadow-soft">
...
const testimonials = [
  { name: "Dr. Sara Ahmed", role: "Pharmacy Owner, Cairo", text: "Parmako cut our monthly stocktake from 2 days to 3 hours. The expiry alerts alone saved us thousands." },
  { name: "Khaled Mansour", role: "Chief Pharmacist", text: "Finally, an app that understands how a pharmacy actually works. The bin system is genius." },
  { name: "Lina Othman", role: "Operations Manager", text: "Clean, fast, and reliable. My team picked it up in one afternoon and never looked back." },
];

const allTestimonials = [
  ...testimonials,
  { name: "Yonas Tesfaye", role: "Pharmacist, Addis Ababa", text: "Inventory accuracy jumped to 99%. Parmako paid for itself in the first month." },
  { name: "Mariam Hassan", role: "Branch Manager", text: "The reorder automation is a lifesaver during busy seasons. Highly recommend." },
  { name: "Ahmed Bekele", role: "Pharmacy Owner", text: "Customer support is fantastic. They actually listen and ship improvements fast." },
  { name: "Hanna Girma", role: "Lead Pharmacist", text: "Switched from spreadsheets and never looked back. Our team loves the bin system." },
  { name: "Samuel Wolde", role: "Operations Lead", text: "Multi-branch sync works flawlessly. We finally have one source of truth." },
  { name: "Rahel Abebe", role: "Owner, 3 branches", text: "Expiry alerts caught batches we would have written off. Real money saved." },
];
...
function MarqueeRow({ items, direction }: { items: typeof allTestimonials; direction: "left" | "right" }) {
    ...
    <section id="testimonials" className="min-h-screen flex items-center py-20 px-6">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground text-lg">Join hundreds of teams already running smarter shelves.</p>
        </div>
        ...
        <div className="mt-16 text-center">
          <p className="text-xs uppercase tracking-widest text-muted-foreground mb-6">Supported by</p>
...
const plans = [
  {
    name: "Basic", price: "4,000", tag: "For small pharmacies",
    features: ["Up to 500 SKUs", "Barcode scanning", "Basic bin management", "Expiry alerts", "Email support"],
    highlight: false,
  },
  {
    name: "Standard", price: "7,000", tag: "Most popular",
    features: ["Up to 5,000 SKUs", "Advanced bin system", "Real-time analytics", "Reorder automation", "Multi-user access (5)", "Priority support"],
    highlight: true,
  },
  {
    name: "Premium", price: "10,000", tag: "For chains & enterprises",
    features: ["Unlimited SKUs", "Multi-branch support", "Audit logs & compliance", "API access", "Unlimited users", "Dedicated account manager", "24/7 support"],
    highlight: false,
  },
];
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map(p => (
            ...
            >
              ...
              <div className={`text-sm mb-2 ${p.highlight ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                {p.highlight ? "Standard plan" : p.tag}
              </div>
              <h3 className="text-2xl font-bold mb-4">{p.name}</h3>
              <div className="flex items-baseline gap-1 mb-6">
                <span className="text-5xl font-bold">{p.price}</span>
                <span className={`text-sm font-semibold ${p.highlight ? "text-primary-foreground/80" : "text-primary"}`}>ETB</span>
                <span className={p.highlight ? "text-primary-foreground/70" : "text-muted-foreground"}>/mo</span>
              </div>
              <Button
              ...
              <ul className="space-y-3">
                {p.features.map(f => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${p.highlight ? "text-primary-foreground" : "text-primary"}`} />
      ...
      <div className="max-w-5xl w-full mx-auto rounded-3xl bg-gradient-deep p-12 md:p-16 text-center shadow-elegant relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <a href="#" className="flex items-center gap-3 px-6 py-3.5 rounded-xl bg-primary-foreground text-primary-deep hover:opacity-90 transition-smooth">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-10 mb-12">
          <div className="lg:col-span-2">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed mb-4 max-w-sm">
              Parmako is the modern pharmacy management platform built to help pharmacies
              of every size track inventory, prevent expirations, and grow with confidence.
              Trusted by 500+ pharmacies across Africa and the Middle East.
            </p>
